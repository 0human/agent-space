...
describe('App Shell preload contract', () => {
  ...
  it('exposes only the controlled runtime information capability', async () => {
    ...
    await api.getProjectWorkflowRun('project-1')
    await api.getWorkflowRun('run-1')
    await api.openWorkflowRunInIde('run-1')
    await api.listRuntimeItems('run-1', 'execution-1')
    ...
    expect(invoke).toHaveBeenCalledWith('workflow-run:get-for-project', 'project-1')
    expect(invoke).toHaveBeenCalledWith('workflow-run:get', 'run-1')
    expect(invoke).toHaveBeenCalledWith('workflow-run:open-in-ide', 'run-1')
    expect(invoke).toHaveBeenCalledWith('runtime-item:list', 'run-1', 'execution-1')
