...
describe('Reasoning in the Runtime Item projection', () => {
  ...
  it('restores active history without rolling back live text and recovers completed sections', () => {
    ...
    projection.restore(history, scope)
    expect(projection.list(scope.executionId)).toEqual([expect.objectContaining({ status: 'in_progress', summary: ['正在检查'], content: ['详细'] })])
    ...
    expect(recovered.list(scope.executionId)).toEqual([expect.objectContaining({ status: 'completed', summary: ['正在'], content: ['详细'] })])
  })
  ...
  it.each(['completed', 'interrupted', 'failed'] as const)('ends the thinking indicator when the Turn is %s without an Item completion', (status) => {
    ...
    expect(projection.list(scope.executionId)).toEqual(expect.arrayContaining([expect.objectContaining({ type: 'reasoning', status: status === 'interrupted' ? 'declined' : status })]))
  })
})
