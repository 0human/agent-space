...
- 当前阶段：MVP 数据库设计
- 目标数据库：SQLite
- 推荐访问层：Drizzle ORM + better-sqlite3
...
| `ai_runtime_config_id` | TEXT | 否 |  | 当前直接绑定 Runtime，仅在 `active_assignee_type = runtime` 时使用 |
...
- `active_assignee_type = team_member` 时，不要求 `ai_runtime_config_id` 落库；实际执行 Runtime 从成员绑定关系解析，并在 `runtime_runs` 中保存快照。
...
| `content` | TEXT | 否 |  | 文本内容、摘要或缓存片段；`local_file` 默认不保存完整正文 |
...
- `type = local_file` 时，默认只保存引用路径、内容哈希、文件大小摘要和可选摘要，不保存完整文件正文。
- 本地文件完整内容只进入单次 `messages.input_envelope_snapshot_json` 的脱敏、截断或摘要快照。
...
- MVP 不支持删除项目主记录后继续保留可恢复历史数据；需要保留历史时应使用归档而不是删除
...
`default_ai_team_id` 与 `default_ai_runtime_config_id` 在项目创建与编辑流程中应保持互斥；同一时刻只允许一个字段有值。
...
- `active_assignee_type = team_member` 时，`ai_runtime_config_id` 不作为必备状态字段；实际执行 Runtime 在启动 Run 时解析。
