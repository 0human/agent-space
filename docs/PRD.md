...
用户在创建项目时不需要先手动选择 `Team 模式` 或 `自由模式`，而是直接选择项目的默认协作对象：`AI Team`、`AI Runtime` 或暂不设置。当项目绑定默认 Team 时，系统自动将其视为 Team 模式；当项目绑定默认 Runtime 或未绑定 Team 时，系统自动将其视为自由模式。在 Team 模式下，用户先创建一个 AI Team，为团队中的不同成员配置角色和 CLI Agent Runtime；项目绑定默认 Team，随后工作窗口可以由 Team 自动分工，也可以由用户手动指定成员执行需求分析、架构设计、代码实现、测试审查、文档整理等任务。在自由模式下，项目不绑定 Team，用户直接手动打开多个工作窗口并选择不同的 CLI Agent Runtime。产品的核心目标是让用户更稳定、更清晰地管理多个 CLI Agent 在多个项目中的协作上下文，而不是把所有任务混在单一聊天窗口里。API Provider 接口作为后续扩展能力保留。
...
- 默认 AI Team 与默认 AI Runtime 不能同时作为项目级默认协作对象生效，默认 AI Team 优先级更高
- 自由模式项目创建后，用户可直接新建工作窗口并手动选择 AI Runtime，或沿用项目默认 Runtime
- 自由模式项目后续可以升级为 Team 模式

#### 9.3.3 编辑项目

用户可以修改项目名称、描述、阶段、本地目录路径、默认 AI Team 和默认 AI Runtime。项目模式由当前默认协作对象自动推导，不作为单独必填字段强制维护。
...
- 当用户选择默认 AI Team 时，系统自动推导为 Team 模式
- 如果所选 Team 的成员 Runtime 缺失或不可用，需要展示警告
- 当用户选择默认 AI Runtime，或未选择默认 Team 时，系统自动推导为自由模式
...
- 用户可以选择手动压缩，后续版本可支持自动压缩
- 压缩结果需要可查看、可引用、可再次发送给 AI
- 用户需要能看到本次压缩保留了哪些决策和问题
- 关键决策、关键问题和关键约束支持“固定保留”或“置顶保留”，避免后续压缩时被弱化
