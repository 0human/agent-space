...
export function ReasoningItem({ item }: { item: RuntimeReasoningItem }): React.JSX.Element {
  const thinking = item.status === 'in_progress'
  return (
    <article className="min-w-0 py-2 text-sm text-muted-foreground [overflow-wrap:anywhere]" aria-label={copy.run.reasoningItem}>
      <div className="flex items-center gap-2" role="status">
        ...
        <span>{copy.run.reasoningStatus[item.status]}</span>
      </div>
