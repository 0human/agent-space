...
- 状态管理：Zustand
- 本地数据库：SQLite
- 数据访问层：Drizzle ORM + better-sqlite3
...
- 项目创建与编辑流程中的默认协作对象为单选；同一时刻只能设置 `defaultAiTeamId` 或 `defaultAiRuntimeConfigId` 其中之一
...
- 解析外部 Runtime 配置文本 / deep link 文本 / 文件
- 通过 Import Adapter 兼容不同外部格式，MVP 首个适配器为 ccswitch Provider
- 生成导入预览
- 执行字段映射
- 冲突检测
- 触发后续 Runtime 检测
...
- 新一轮 `session:sendMessage` 仍然创建新的 `runtime_runs`
- 是否透传 `resumeExternalSessionId` 由对应 Runtime Adapter 按 CLI 能力决定
- 该 ID 仅作为恢复线索，不作为应用主状态来源
- `waiting_input` 只表示 CLI 需要用户补充信息，不强制要求复用同一个子进程 stdin
- MVP 不统一要求交互式 stdin 续写；若某个 CLI Adapter 后续支持，也必须保持 `runtime_runs` 独立记录
...
触发后系统只提示用户处理，不静默自动压缩。用户需要明确选择减少引用、改为片段引用或执行压缩。
...
- `summary` 用于人类快速浏览
- `decision_summary_json` 存关键决策和关键结论
...
- 关键决策、关键问题、关键结论和关键约束可被标记为 pinned
- pinned 内容在后续压缩中必须优先保留
...
- 如果用户取消压缩，系统不得改写当前上下文，只能提示减少引用或改用片段
...
`context_items` 用于保存可复用上下文资产和引用记录，不作为本地文件全文仓库。

- `type = local_file` 时默认只保存路径、标题、内容哈希、文件大小和可选摘要
- 本地文件完整内容仅在用户显式选择发送时进入本轮 `RuntimeInputEnvelope`
- `messages.input_envelope_snapshot_json` 保存本轮实际发送内容的脱敏、截断或摘要快照
- 大文件默认只发送片段或摘要；完整注入需要用户明确确认
- 普通项目资料、手动文本和历史产出可以按需保存正文内容
...
- 若首个工作窗口创建失败：
  - 项目仍保留
  - UI 返回 warning，提示“项目已创建，首个工作窗口未完成”

## 13. 外部 Runtime 配置导入设计
...
- 应用只解析用户提供的内容，不主动调用 deep link 协议，不打开外部应用。
- ccswitch 是 MVP 首个兼容格式，但不是导入架构的唯一目标。
...
```ts
interface RuntimeImportAdapter {
  sourceFormat: RuntimeImportFormat;
  detect(input: RuntimeImportRawInput): Promise<RuntimeImportDetectResult>;
  preview(input: RuntimeImportRawInput): Promise<RuntimeImportPreview[]>;
  commit(input: RuntimeImportCommitInput): Promise<RuntimeImportCommitResult>;
}
```
...
1. 用户输入原始内容
2. `ImportService` 自动识别格式，或使用用户提供的 `formatHint`
3. 解析 Provider 配置
...
- 推荐：进入首版承诺范围
- 原因：目标用户已明确包含 Gemini CLI 重度使用者，且 Provider 枚举与导入模型已预留，首版直接支持可减少后续迁移成本
