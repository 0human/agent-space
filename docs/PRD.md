...
用户在创建项目时不需要先手动选择 `Team 模式` 或 `自由模式`，而是直接选择项目的默认协作对象：`AI Team`、`AI Runtime` 或暂不设置。默认协作对象在创建向导中为单选：同一时刻只能选择 Team、Runtime 或留空其中一种。当项目绑定默认 Team 时，系统自动将其视为 Team 模式；当项目绑定默认 Runtime 或未绑定 Team 时，系统自动将其视为自由模式。在 Team 模式下，用户先创建一个 AI Team，为团队中的不同成员配置角色和 CLI Agent Runtime；项目绑定默认 Team，随后工作窗口可以由 Team 自动分工，也可以由用户手动指定成员执行需求分析、架构设计、代码实现、测试审查、文档整理等任务。在自由模式下，项目不绑定 Team，用户直接手动打开多个工作窗口并选择不同的 CLI Agent Runtime。产品的核心目标是让用户更稳定、更清晰地管理多个 CLI Agent 在多个项目中的协作上下文，而不是把所有任务混在单一聊天窗口里。API Provider 接口作为后续扩展能力保留。
...
Agent 公用配置用于抽取多个 Runtime、AI Team 成员和项目可复用的行为配置，避免在每个 Runtime 中重复维护相同的权限、基础 Prompt 和默认参数。
...
- AI Runtime 可以引用一个 Agent 公用配置
- AI Team 成员可以引用一个 Agent 公用配置
- 项目可以指定默认 Agent 公用配置
- 工作窗口可以继承或临时覆盖 Agent 公用配置

### 6.3 项目

项目是 AI 工作的组织单位。一个项目可以关联本地目录、项目说明、阶段状态、文档资料、可选的默认 AI Team 和多个工作窗口。

项目支持两种模式，且由默认协作对象自动推导：

- Team 模式：项目绑定默认 AI Team，新建工作窗口时默认继承 Team 或 Team 成员
- 自由模式：项目绑定默认 AI Runtime，或不绑定 Team，用户手动创建多个工作窗口并分别选择 AI Runtime
...
一个 AI Team 可以被多个项目复用。项目在创建时可以选择默认 Team，也可以改为选择默认 Runtime 或跳过 Team 进入自由模式。后续任务可以继承该 Team，也可以手动覆盖。
...
用户可以创建一个 Agent 公用配置，预先定义常用权限、基础 Prompt 和默认参数，并在多个 Runtime、Team 成员和项目之间复用。
...
用户创建一个新项目，填写项目名称、项目描述和本地项目目录，并选择项目的默认协作对象：

- 选择一个默认 AI Team，系统自动推导为 Team 模式
- 选择一个默认 AI Runtime，或不选择 Team，系统自动推导为自由模式
...
- Runtime 的默认命令参数不是必填项；未填写时按空参数数组处理

配置合并规则：

- Runtime 可直接定义自身权限策略和系统提示词
- Runtime 也可以引用一个 Agent 公用配置作为基础模板
- 当 Runtime 自身字段与 Agent 公用配置冲突时，Runtime 自身配置优先
...
用户可以编辑、复制已有 Agent 公用配置。

交互要求：

- 支持“另存为模板”
- 支持查看哪些 Runtime、Team 成员、项目正在引用该配置
- 修改后，新建任务默认使用新配置；已存在工作窗口是否同步更新由用户确认

#### 9.2.4 删除配置

用户可以删除 Agent 公用配置。如果该配置正在被 Runtime、Team 成员、项目或工作窗口引用，需要提示影响范围。
...
- 选择默认 AI Team 时，系统自动推导为 Team 模式
- 选择默认 AI Runtime 时，系统自动推导为自由模式
- 创建向导中默认 AI Team 与默认 AI Runtime 只能单选，不能同时设置
- 自由模式项目创建后，用户可直接新建工作窗口并手动选择 AI Runtime，或沿用项目默认 Runtime
- 自由模式项目后续可以升级为 Team 模式

#### 9.3.3 编辑项目

用户可以修改项目名称、描述、阶段、本地目录路径和默认协作对象。项目模式由当前默认协作对象自动推导，不作为单独必填字段强制维护。编辑时默认协作对象同样只能单选。
...
- MVP 不支持“仅删除项目记录但保留可恢复历史数据”的孤儿数据模式，避免产生失效外键和不可追溯历史
...
创建项目流程需要支持 Team 模式和自由模式两条路径，并保证新用户可以通过“默认协作对象”快速完成首次项目创建，而不需要先选择抽象模式。
...
- 展示可选 Runtime 列表，包括配置名称、CLI 类型、最近使用时间和可用状态
- 支持在当前流程中快捷新建 AI Team
- 支持在当前流程中快捷新建 AI Runtime
- 当用户选择默认 AI Team 时，系统自动推导为 Team 模式，并清空默认 Runtime
- 如果所选 Team 的成员 Runtime 缺失或不可用，需要展示警告
- 当用户选择默认 AI Runtime 时，系统自动推导为自由模式，并清空默认 Team
- 当用户未选择默认协作对象时，系统自动推导为自由模式
- UI 中需要明确展示系统推导出的项目模式，作为只读结果，而不是要求用户先选择模式
- 展示说明：自由模式下可直接打开多个工作窗口并手动选择不同 Runtime
...
- Team 模式下，如果用户选择“创建后打开首个工作窗口”，应用优先建议使用 Team 中的默认成员。默认成员规则为：先按 `defaultLaunchMode` 匹配角色；若没有匹配成员，则选择 `enabled = true` 且 `sortOrder` 最小的成员
...
- 如果已选择默认 Team，但 Team 被删除或不可用，需要要求用户重新选择 Team 或切换到默认 Runtime
- 如果已选择默认 Runtime，但 Runtime 不可用，需要要求用户重新选择 Runtime 或清空默认 Runtime
- 如果没有任何可用 Runtime，应用需要提示先去创建 AI Runtime 配置
- 如果用户中途取消创建，不应产生半成品项目记录
- 如果选择“创建后打开首个工作窗口”且首个工作窗口创建失败，项目记录保留，并向用户返回 warning
...
- 如果用户已有 Team，创建项目时默认高亮最近使用的 Team；如果没有 Team，则默认高亮最近使用的 Runtime
...
- Team 不单独维护显式 `is_default` 字段
- 创建首个工作窗口或按 Team 默认建议成员时，先按 `defaultLaunchMode` 匹配角色
- 若无匹配角色，则选择 `enabled = true` 且 `sortOrder` 最小的成员作为默认建议成员
...
- 如果项目已绑定默认 AI Team，新建工作窗口时默认继承该 Team。用户可以直接使用 Team 默认成员，也可以手动指定某个成员执行。默认成员规则为：先按 `defaultLaunchMode` 匹配角色；若没有匹配成员，则选择 `enabled = true` 且 `sortOrder` 最小的成员
...
应用不能只展示用户输入的一句文本，还需要让用户理解“本轮实际发送给 CLI Agent 的完整输入包”。
...
- 输入摘要默认可见，完整输入包默认折叠
- 用户需要明确知道本轮发送了哪些文件和资料
- 敏感字段不得明文展示，例如 API Key、Token、敏感环境变量
- 系统拼装后的最终 prompt 或 payload 可以查看，但不应默认展开影响主消息流阅读
...
AI 返回内容需要以“可读优先、细节可展开”为原则展示，避免把底层 CLI 日志直接混入主消息流。
...
- 主消息流默认只强调可读答案，不让工具日志打断阅读
- Team 模式下每条 AI 回复都需要明确标记所属成员
- 如果是失败结果，需要优先显示用户可理解的错误摘要，再允许展开查看原始错误
- 如果是等待用户输入或等待权限确认，需要在输出中明确说明当前阻塞原因和下一步动作
- 工具调用、文件变更和运行日志默认折叠显示
...
- `local_file` 类型上下文默认只保存文件引用信息，包括路径、标题、内容哈希、文件大小和可选摘要
- 本地文件完整内容不默认写入 `context_items.content`
- 文件完整内容仅在用户显式选择发送时进入本轮输入包
- 本轮实际发送内容需要写入消息的输入包快照，且进入展示或日志前必须脱敏、截断或摘要化
...
当上下文接近或超过限制时，应用不能静默自动压缩。系统需要先提示用户当前上下文过大，并提供减少引用、改为片段引用或手动压缩等选项。

#### 9.7.5 上下文压缩策略

当工作窗口的历史消息、引用文件和项目资料过多，导致上下文接近模型限制时，应用不能只做简单截断，而应优先生成结构化压缩上下文，并保留关键决策与关键问题。
...
建议将压缩结果组织为结构化摘要，而不是单段自由文本。推荐结构包括：
...
- 当系统准备压缩上下文时，需要明确提示用户当前上下文过大
- 用户确认后才允许执行压缩，MVP 不做静默自动压缩
- 压缩结果需要可查看、可引用、可再次发送给 AI
- 用户需要能看到本次压缩保留了哪些决策、问题和结论
- 关键决策、关键问题、关键结论和关键约束支持“固定保留”或“置顶保留”，避免后续压缩时被弱化
- 压缩结果必须保留关键问题、关键结论、关键决策、关键约束和必要来源引用

压缩后的结果应作为项目或工作窗口中的可复用上下文资产保存，供后续消息、转交任务和关联窗口继续引用。
...
- `WorkSession` 和 `runtime_runs` 是应用侧权威状态，不能完全依赖外部 CLI 自己的会话状态
- 每次 `session:sendMessage` 都需要创建新的 `runtime_runs` 记录，用于保存本次真实执行、诊断和恢复信息
- 如果目标 CLI 支持 `external_session_id`、resume 参数或等价能力，由对应 Runtime Adapter 决定是否透传 `resumeExternalSessionId`
- `waiting_input` 表示当前 CLI 需要用户补充信息；用户补充后是否复用外部 CLI 会话，由对应 Runtime Adapter 按 CLI 能力决定
- MVP 不要求所有 CLI 都支持同一进程 stdin 续写；交互式 stdin 续写可以作为特定 Adapter 的后续增强能力
...
- 顶部显示当前步骤和总步骤数
- 每一步只展示与当前决策相关的字段
- Team 与 Runtime 的选择变化时，系统推导的项目模式需要即时变化
- 关键字段错误需要在当前步骤内即时提示
...
- 用户消息下方需要显示本轮输入摘要，例如引用文件数量、项目资料数量、当前成员、Runtime、工作目录和权限级别
- 右侧面板或消息详情中需要提供“查看完整输入包”入口
- AI 回复默认显示可读正文，并在展开区展示工具调用、文件变更、权限请求、命令执行记录和运行日志
- 失败回复需要优先展示人类可理解的错误摘要，并提供查看原始错误输出的入口
...
interface AIRuntimeConfig {
  ...
  provider: 'claude_code_cli' | 'codex_cli' | 'gemini_cli' | 'custom_cli';
  ...
  permissionPreset: 'read_only' | 'project_write' | 'command_approval' | 'full_access';
  isDefault: boolean;
  createdAt: string;
  updatedAt: string;
  lastUsedAt?: string;
...
- `permissionPreset` 仅表示快捷预设和 UI 摘要，最终权限应由可组合权限设置合并得到
...
- `permissionPreset` 仅用于快捷初始化和展示摘要，不作为最终权限来源
...
interface Project {
  ...
  recentRuntimeType?: 'claude_code_cli' | 'codex_cli' | 'gemini_cli' | 'custom_cli';
...
- Team 模式与自由模式并存后，项目配置入口和窗口选择逻辑会更复杂，需要通过“默认协作对象”降低用户理解成本。
